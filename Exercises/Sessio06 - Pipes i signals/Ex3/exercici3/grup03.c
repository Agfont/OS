...
int main(void)
{
    ...
    if (ret == 0) { // child      	
        ...
        while (i < num_values) {
            ...
            read(fd[0], &value, sizeof(value));//reutiliza el value para sumar
            sum = sum + value;
            i++;
        }
        ...
        write(fd[1], &sum, sizeof(sum));//write the data
        ...
        num_values = MAXVALUE;
        write(fd[1], &num_values, sizeof(num_values));//write the data
        
        while (i < num_values) {  
            /* Generar valor aleatori i inserir a la canonada */
            value = rand() % MAXVALUE + 1;
            write(fd[1], &value, sizeof(value));//write the data
            i++;
        }                
        sleep(1);
