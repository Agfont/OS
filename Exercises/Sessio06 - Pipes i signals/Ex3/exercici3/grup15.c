...
int main(void)
{
	...
	printf("El pare escriu a la canonada!\n"); // (A) 
        num_values = 1000;
        write(fd[1], &num_values, 1); //(1) Inserir valor a la canonada
        while (i < num_values) {  
            /* Generar valor aleatori i inserir a la canonada */
            value = rand() % MAXVALUE + 1;
	    write(fd[1], &value, 1); // (1) Inserir valor a la canonada
            i++;
        }
