...
int main(void)
{
    ...
    { // child
        ...
        /* Codi per gestionar senyals */
        signal(SIGUSR2,sigusr2); //Assignem la funció sigusr2 a la senyal SIGUSR2
        parent_pid = getppid(); //Obtenim PID del pare
    ...
    { // parent
        ...
        /* Codi per gestionar senyals */
        child_pid = ret; //Guardem el PID del fill
        signal(SIGUSR1,sigusr1); //Assignem la funció sigusr1 a la senyal SIGUSR1
        /* Random seed */
        srand(time(NULL));
